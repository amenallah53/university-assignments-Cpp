...
//remplire les données d'une instance d'ARTICLE
void ARTICLE::lire_article() {

    cout << "Code: ";
    cin >> code;
    if (code <= 0) throw invalid_argument("Le code doit être un entier positif.");

    cout << "Designation: ";
    cin.ignore();
    getline(cin, designation);
    if (designation.empty()) throw invalid_argument("La designation ne peut pas être vide.");

    cout << "Quantité en stock: ";
    cin >> quantiteStock;
    if (quantiteStock < 0) throw invalid_argument("La quantité en stock ne peut pas être négative.");
}
...
//remplire les données d'une instance d'PRODUIT_FINI
void PRODUIT_FINI::lire_produit_fini() {
    
    lire_article();
    cout << "Marque: ";
    cin.ignore();
    getline(cin, marque);
    if (marque.empty()) throw invalid_argument("La marque ne peut pas être vide.");

    cout << "Prix de revient: ";
    cin >> prixRevient;
    if (prixRevient < 0.0) throw invalid_argument("Le prix de revient ne peut pas être négatif.");

    cout << "Nombre d'opérations: ";
    cin >> nombreOperations;
    if (nombreOperations < 0 || nombreOperations > 10)
        throw out_of_range("Le nombre d'opérations doit être entre 0 et 10.");
